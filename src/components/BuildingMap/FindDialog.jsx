...
const FindDialog = ({ data, goBuilding, toggleVisible, city }) => {
  ...
  const cssStyle = (data) => {
    var divHeight;
    var obj = document.getElementById(`${data.name}_${data.floor}`);
        ...
        {!selectBuild ? (
          ...
          <div className='flex flex-wrap w-full justify-around items-center'>
            {selectBuild.floors.map((elem, index) => (
              <div key={index} className='flex flex-col border-2 border-[#008954] mb-[30px] h-[320px]'>

                <div className='font-semibold bg-[#00895499] text-center'>
                  <p>{elem.floor} этаж</p>
                </div>

                <div className={elem.zone.length > 5 && 'columns-2 py-2'}>

                  {elem.zone.map((zone, index) => (
                    <button onClick={() => { goBuilding([selectBuild.name, index, elem.floor], 'find', zone); setSelectBuild() }} className="w-[350px] m-[10px] flex justify-center border-2 mb-1" key={index}>
                      <p className='text-[18px] font-serif break-words'>{zone.name}</p>
                    </button>
                  ))}
